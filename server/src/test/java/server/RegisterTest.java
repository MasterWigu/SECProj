...
public class RegisterTest extends ServerTestsBase{
    ...
    public void success() throws UserNotFoundException, KeyException, InvalidWtsException {
        ...
        int id1 = Integer.parseInt(server.register(client1Keys.getPublic(), wts1+1).replace("Successfully logged in, your id is ", ""));
        int wts2 = server.getRegisterWts();
        Assert.assertEquals(wts2, wts1+1);
        int id2 = Integer.parseInt(server.register(client2Keys.getPublic(), wts2+1).replace("Successfully logged in, your id is ", ""));
        int wts3 = server.getRegisterWts();
        Assert.assertEquals(wts3, wts2+1);
        int id3 = Integer.parseInt(server.register(client3Keys.getPublic(), wts3+1).replace("Successfully logged in, your id is ", ""));
    ...
    public void duplicateUser() throws UserNotFoundException, KeyException, InvalidWtsException {
        ...
        int wts2 = server.getRegisterWts();
        Assert.assertEquals(wts2, wts1+1);
        int id2 = Integer.parseInt(server.register(client1Keys.getPublic(), wts2+1).replace("Successfully logged in, your id is ", ""));
